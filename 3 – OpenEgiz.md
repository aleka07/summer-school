...
OpenEgiz — это наша платформа для цифровых двойников. Она построена вокруг OpenTwins и Eclipse Ditto: Ditto хранит и обновляет состояние двойников, OpenTwins дает удобный интерфейс для работы с ними, Grafana показывает данные, а InfluxDB хранит временные ряды.
...
На схеме видно главный поток: устройства или генераторы данных отправляют значения через MQTT, дальше эти данные попадают в Ditto, сохраняются в InfluxDB и показываются в Grafana. OpenTwins работает как интерфейс управления цифровыми двойниками.
...
При первой установке MongoDB иногда не успевает запуститься до `ditto-extended-api`. Из-за этого Grafana или OpenTwins могут не подключиться нормально. В этом случае перезапускаем deployment:
...
Когда все поднялось, открываем Grafana/OpenTwins по публичному IP нашего сервера и порту `30718`.
...
В конце занятия у нас должна быть рабочая установка OpenEgiz: Kubernetes поднят, OpenEgiz установлен, Grafana открывается, OpenTwins доступен.
